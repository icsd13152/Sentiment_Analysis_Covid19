# import dash
import itertools

import dash as d
# import dash as html
import re
import joblib
import nltk
import numpy as np
from nltk.corpus import stopwords, wordnet
from nltk.stem import WordNetLemmatizer
import demoji
import pandas as pd
import plotly.express as px
from sklearn.manifold import TSNE

data = pd.read_csv(r"Datasets/Corona_NLP_test.csv")
print(data.shape)

tfidf_vectorizer = joblib.load('savedModels/vectorizer.sav')



#short words/apostrophe lookup
contraction_dict1 = {"Â":"","’":"'","Ã":""}
contraction_dict2 = {"Â":"","’":"'","ain't": "are not","'s":" is","aren't": "are not","don't": "do not","Don't":"Do not",
                     "I'll":"I will","Didn't":"Did not","hasn't":"has not","NYC":"New York City","16MAR20":"",
                     "I'd":"I would","I've":"I have","you're":"you are","I'm":"I am","it's":"it is",
                     "#NZ":"","they'll":"they will","they're":"they are","can't":"can not","Y'all":"You All",
                     "I m":"I am","can't":"can not","don t":"do not","I ve":"I have","we're":"we are",
                     "LOL":"lough out loud","lol":"lough out loud","FYI":"For your information","OFC":"Of Course","ofc":"Of Course",
                     "#coronavirÃ¼s":"coronavirus","pls":"please","#stayhomesavelives":"stay home save lives",
                     "hasn't": 'has not',"haven't": 'have not',"he'd": 'he had / he would',"he'd've": 'he would have',
                     "he'll": 'he shall / he will',"he'll've": 'he shall have / he will have',
                     "he's": 'he has / he is',"how'd": 'how did', "how'd'y": 'how do you', "how'll": 'how will',
                     "how's": 'how has / how is', "i'd": 'I had / I would',"i'd've": 'I would have',  "i'll": 'I shall / I will',
                     "i'll've": 'I shall have / I will have',"i'm": 'I am', "i've": 'I have', "isn't": 'is not', "it'd": 'it had / it would',
                     "it'd've": 'it would have', "it'll": 'it shall / it will',
                     "it'll've": 'it shall have / it will have',"it's": 'it has / it is', "let's": 'let us',
                     "ma'am": 'madam', "mayn't": 'may not',
                     "might've": 'might have', "mightn't": 'might not',
                     "mightn't've": 'might not have', "must've": 'must have',"mustn't": 'must not',
                     "mustn't've": 'must not have', "needn't": 'need not',
                     "needn't've": 'need not have', "o'clock": 'of the clock',
                     "oughtn't": 'ought not', "oughtn't've": 'ought not have',
                     "shan't": 'shall not', "sha'n't": 'shall not',
                     "shan't've": 'shall not have', "she'd": 'she had / she would',
                     "she'd've": 'she would have', "she'll": 'she shall / she will',
                     "she'll've": 'she shall have / she will have',
                     "she's": 'she has / she is', "should've": 'should have',
                     "shouldn't": 'should not',"shouldn't've": 'should not have',
                     "so've": 'so have', "so's": 'so as / so is',
                     "that'd": 'that would / that had',"that'd've": 'that would have',
                     "that's": 'that has / that is', "there'd": 'there had / there would',
                     "there'd've": 'there would have', "there's": 'there has / there is',
                     "they'd": 'they had / they would',  "they'd've": 'they would have',
                     "they'll": 'they shall / they will', "they'll've": 'they shall have / they will have',
                     "they're": 'they are',  "they've": 'they have',
                     "to've": 'to have', "wasn't": 'was not',
                     "we'd": 'we had / we would',  "we'd've": 'we would have',
                     "we'll": 'we will', "we'll've": 'we will have',
                     "we're": 'we are', "we've": 'we have',
                     "weren't": 'were not', "what'll": 'what shall / what will',
                     "what'll've": 'what shall have / what will have',
                     "what're": 'what are', "what's": 'what has / what is',
                     "what've": 'what have',"when's": 'when has / when is',
                     "when've": 'when have', "where'd": 'where did',
                     "where's": 'where has / where is',
                     "where've": 'where have', "who'll": 'who shall / who will',
                     "who'll've": 'who shall have / who will have',
                     "who's": 'who has / who is', "who've": 'who have',
                     "why's": 'why has / why is', "why've": 'why have',
                     "will've": 'will have', "won't": 'will not',"won't've": 'will not have',
                     "would've": 'would have',"wouldn't": 'would not',"wouldn't've": 'would not have',
                     "y'all": 'you all', "y'all'd": 'you all would',
                     "y'all'd've": 'you all would have', "y'all're": 'you all are',
                     "y'all've": 'you all have', "you'd": 'you had / you would',
                     "you'd've": 'you would have',"&amp":"and","btc":"bitcoin","irs":"","spx":"","📍":"","✅":"","ive":"i have",
                     "coo":"","lka":"", "nyc":"","ktla":"","ppc":"pay per click","wjhl":"","plzzz":"please","orlf":"","etc":"",
                     "ktvu":"","amidst":"","biz":"business","djt":"","ict":"information communications technology","yep":"yes",
                     "yeap":"yes","gov":"goverment","psa":"public service announcement"
                     }

emoticons={':)': 'happy', ':‑)': 'happy',
           ':-]': 'happy', ':-3': 'happy',
           ':->': 'happy', '8-)': 'happy',
           ':-}': 'happy', ':o)': 'happy',
           ':c)': 'happy', ':^)': 'happy',
           '=]': 'happy', '=)': 'happy',
           '<3': 'happy', ':-(': 'sad',
           ':(': 'sad', ':c': 'sad',
           ':<': 'sad', ':[': 'sad',
           '>:[': 'sad', ':{': 'sad',
           '>:(': 'sad', ':-c': 'sad',
           ':-< ': 'sad', ':-[': 'sad',
           ':-||': 'sad','😢':'sad'}

myOwnStopWords={'price':"",
                'store':"",
                'supermarket':"",
                'food':"",
                'grocery':"",
                'people':"",
                'go':"",
                'consumer':""}
#

def lookup_dict(text, dictionary):
    for word in text.split():
        if word.lower() in dictionary:
            if word.lower() in text.split():
                text = text.replace(word, dictionary[word.lower()])
    return text

def get_pos( word ):
    w_synsets = wordnet.synsets(word)

    pos_counts = nltk.Counter()
    pos_counts["n"] = len(  [ item for item in w_synsets if item.pos()=="n"]  )
    pos_counts["v"] = len(  [ item for item in w_synsets if item.pos()=="v"]  )
    pos_counts["a"] = len(  [ item for item in w_synsets if item.pos()=="a"]  )
    pos_counts["r"] = len(  [ item for item in w_synsets if item.pos()=="r"]  )

    most_common_pos_list = pos_counts.most_common(3)
    return most_common_pos_list[0][0]

def process(df):

    df["Sentiment"] = df["Sentiment"].replace('Extremely Negative', 'Negative', regex=True)

    df["Sentiment"] = df["Sentiment"].replace('Extremely Positive', 'Positive', regex=True)
    df["Sentiment"]=df["Sentiment"].replace('Negative', -1, regex=True)
    df["Sentiment"]=df["Sentiment"].replace('Positive', 1, regex=True)
    df["Sentiment"]=df["Sentiment"].replace('Neutral', 0, regex=True)
    #to lower case
    # df["OriginalTweet"]=df["OriginalTweet"].apply(lambda x:demoji.replace(x))
    df['OriginalTweet']  = df['OriginalTweet'].str.lower()
    df["OriginalTweet"] = df["OriginalTweet"].apply(lambda x: lookup_dict(x,emoticons))

    df['OriginalTweet']=df['OriginalTweet'].apply(lambda x:lookup_dict(x,contraction_dict1))
    df['OriginalTweet']=df['OriginalTweet'].apply(lambda x:lookup_dict(x,contraction_dict2))

    # df['OriginalTweet'] = df['OriginalTweet'].apply(lambda x: ''.join(''.join(s)[:2] for _, s in itertools.groupby(x)))


    #to lower case
    df['OriginalTweet']  = df['OriginalTweet'].str.lower()

    #remove numbers
    df["OriginalTweet"] = df["OriginalTweet"].replace('[0-9]', '', regex=True)

    #remove mentions
    df["OriginalTweet"] = df["OriginalTweet"].replace('@([a-zA-Z0-9_]{1,50})', '', regex=True)

    #remove hashtags
    df["OriginalTweet"] = df["OriginalTweet"].replace('#', '', regex=True)

    #remove urls
    df["OriginalTweet"] = df["OriginalTweet"].replace('http\S+', '', regex=True)

    # # #remove all remaining bad chars
    df["OriginalTweet"]=df["OriginalTweet"].replace('[^\\w\\s]', '', regex=True)

    df["OriginalTweet"]=df["OriginalTweet"].replace("Â", "", regex=True)
    df["OriginalTweet"]=df["OriginalTweet"].replace("â", "a", regex=True)
    df["OriginalTweet"]=df["OriginalTweet"].replace("nz", "", regex=True)
    df['OriginalTweet']  = df['OriginalTweet'].str.strip()


    df["OriginalTweet"]=df["OriginalTweet"].replace("_", "", regex=True)
    df["OriginalTweet"]=df["OriginalTweet"].replace("   ", " ", regex=True)
    df["OriginalTweet"]=df["OriginalTweet"].replace("  ", " ", regex=True)
    # df["OriginalTweet"]=df["OriginalTweet"].replace("rn", "", regex=True)
    # df["OriginalTweet"]=df["OriginalTweet"].replace("vr", "", regex=True)
    # df["OriginalTweet"]=df["OriginalTweet"].replace("hl", "", regex=True)
    # df["OriginalTweet"]=df["OriginalTweet"].replace("u", "", regex=True)
    # df["OriginalTweet"]=df["OriginalTweet"].replace("ot", "", regex=True)
    df['OriginalTweet']  = df['OriginalTweet'].str.strip()
    df["OriginalTweet"]=df["OriginalTweet"].replace("coronavir¼", "coronavirus", regex=True)
    df["OriginalTweet"]=df["OriginalTweet"].replace("pmmodi", "", regex=True)
    df["OriginalTweet"]=df["OriginalTweet"].replace("amp", "and", regex=True)
    df["OriginalTweet"]=df["OriginalTweet"].fillna(0)

    df['OriginalTweet']  = df['OriginalTweet'].str.strip()
    # print(df["OriginalTweet"][16])
    # print(df["OriginalTweet"].head(25))
    #Tokenize the tweets
    tokenized_tweets = df["OriginalTweet"].apply(lambda x: x.split())

    #remove stopword(for example and,to at etc)
    stop_words = set(stopwords.words('english'))
    tokenized_tweets = tokenized_tweets.apply(lambda x: [word for word in x if not word in stop_words])

    # stemmer = PorterStemmer()#language='english'
    # tokenized_tweets= tokenized_tweets.apply(lambda x: [stemmer.stem(i) for i in x])

    lemmatizer = WordNetLemmatizer()
    tokenized_tweets = tokenized_tweets.apply(lambda x: [lemmatizer.lemmatize(i,get_pos( i )) for i in x])
    tokenized_tweets = tokenized_tweets.apply(lambda x: [demoji.replace(i) for i in x])
    tokenized_tweets = tokenized_tweets.apply(lambda x: [word for word in x if len(word)>2 or word=='go'])
    # tokenized_tweets= tokenized_tweets.apply(lambda x: [stemmer.stem(i) for i in x])

    #Joining the tokenized tweets
    for i in range(len(tokenized_tweets)):
        tokenized_tweets[i] = ' '.join(tokenized_tweets[i])
    df["OriginalTweet"] = tokenized_tweets


    # df['OriginalTweet']=df['OriginalTweet'].apply(lambda x:lookup_dict(x,myOwnStopWords))
    all_words = []
    for line in list(df['OriginalTweet']):
        words = line.split()
        for word in words:
            all_words.append(word.lower())


    df["OriginalTweet"]=df["OriginalTweet"].replace("  ", " ", regex=True)
    df['OriginalTweet']  = df['OriginalTweet'].str.strip()
    df['OriginalTweet'] = df['OriginalTweet'].replace(' +', ' ')
    df.drop_duplicates(subset ="OriginalTweet",
                         keep = 'last', inplace = True)
    df.reset_index(drop=True, inplace=True)
    # df['OriginalTweet'].replace('', 'NaN', inplace=True)
    # # df.dropna(subset=['OriginalTweet'], inplace=True)
    # df = df[df['OriginalTweet'] != 'NaN']
    # print(len(df))
    # for i in range(len(df["OriginalTweet"])):
    #     print(df["OriginalTweet"][i])
    print(df.shape)
    tfidf= tfidf_vectorizer.transform(df["OriginalTweet"])
    print(tfidf.shape)
    return (tfidf,df)


def processSingle(tweet):

    lookup_dict(tweet,emoticons)
    lookup_dict(tweet,contraction_dict1)
    lookup_dict(tweet,contraction_dict2)
    tweet = tweet.lower()
    tweet = re.sub(r'[0-9]','',tweet)
    tweet = re.sub(r'@([a-zA-Z0-9_]{1,50})','',tweet)
    tweet = re.sub(r'http\S+','',tweet)

    # tweet = tweet.replace(r'[0-9]', '')
    # tweet = tweet.replace(r'@([a-zA-Z0-9_]{1,50})', '')
    tweet = tweet.replace('#', '')
    # tweet = tweet.replace(r'http\S+', '')
    tweet = tweet.replace("_", "")
    tweet = tweet.replace("-", "")
    tweet = tweet.strip()
    tokenized_tweets = tweet.split()




    #remove stopword(for example and,to at etc)
    stop_words = set(stopwords.words('english'))
    lemmatizer = WordNetLemmatizer()
    for i in range(len(tokenized_tweets)):
        if tokenized_tweets[i] not in stop_words:
            # token = demoji.replace(i)
            token = lemmatizer.lemmatize(tokenized_tweets[i],get_pos( tokenized_tweets[i] ))
            if(len(token)>2):
                tokenized_tweets[i] = token


    # tokenized_tweets= tokenized_tweets.apply(lambda x: [stemmer.stem(i) for i in x])

    #Joining the tokenized tweets
    for i in range(len(tokenized_tweets)):
        tokenized_tweets[i] = ''.join(tokenized_tweets[i])
    tweet = ' '.join(tokenized_tweets)
    print(tokenized_tweets)



    tfidf= tfidf_vectorizer.transform([tweet])
    # print(tfidf,tweet)
    return (tfidf,tweet)

# def prediction(mymodel,tftweets):
#     predicted = mymodel.predict(tftweets)
#
#
#
#     # df = pd.DataFrame(columns=['OriginalTweet','OriginalSentiment','PredictedSentiment'])
#
#     data['predictedSentiment'] = predicted
#
#     c =0
#     for i in range(len(predicted)):
#         if predicted[i] == 0:
#             print(predicted[i],data['OriginalTweet'][i])
#             c = c +1
#     print(c)
#     data["predictedSentiment"]=data["predictedSentiment"].replace(-1,'Negative', regex=True)
#     data["predictedSentiment"]=data["predictedSentiment"].replace( 1,'Positive', regex=True)
#     data["predictedSentiment"]=data["predictedSentiment"].replace( 0,'Neutral', regex=True)
#     return data['predictedSentiment']


tweets,processeddf = process(data)
tsne = TSNE(n_components=1,learning_rate='auto',init='random')
X_embedded  = tsne.fit_transform(tweets)
print(tweets.shape)
print(processeddf.shape)
value = input("Please enter 1 if you want your single input or 2 from saved csv:\n")

if value == '2':


    print("1: SVM")
    print("2: Logistic Regression")
    print("3: Naive Bayes")
    m = input("Please Choose the number of model you want:\n")
    loaded_model = None


    if m == '1':
        loaded_model = joblib.load('savedModels/SVC.sav')

    elif m == '3':
        loaded_model = joblib.load('savedModels/nb.sav')

    elif m == '2':
        loaded_model = joblib.load('savedModels/LR.sav')

    predicted = loaded_model.predict(tweets)

    processeddf["PredictedSentiment"] = predicted
    countNegative = 0
    countPositive = 0
    countNeutral = 0

    for i in range(len(predicted)):
        # for j in range(len(processeddf["Sentiment"])):
            if predicted[i] == processeddf["Sentiment"][i]:
                if predicted[i] == -1:
                    countNegative = countNegative + 1
                elif predicted[i] == 1:
                    countPositive = countPositive + 1
                elif predicted[i] == 0:
                    countNeutral = countNeutral + 1


    # for i in range(len(processeddf["Sentiment"])):
    #     print(processeddf["Sentiment"][i])

    print(countNeutral)
    print("Found correct Positives "+str(countPositive) + " from "+ str(len(processeddf[processeddf["Sentiment"]==1])))
    print("Found correct Neutrals "+str(countNeutral) + " from "+ str(len(processeddf[processeddf["Sentiment"]==0])))
    print("Found correct Negatives "+str(countNegative) + " from "+ str(len(processeddf[processeddf["Sentiment"]==-1])))
    fig = px.bar(x=['Negative','Actual Negative','Neutral','Actual Neutral','Positive','Actual Positive'],
           y=[countNegative,len(processeddf[processeddf["Sentiment"]==-1]),countNeutral,len(processeddf[processeddf["Sentiment"]==0]),
              countPositive,len(processeddf[processeddf["Sentiment"]==1])])
    fig.show()

    probas = loaded_model.predict_proba(tweets)


    df = pd.DataFrame(columns=['tsne_1','probas','Features'])
    df['tsne_1'] = X_embedded[:, 0]
    df['probas'] = pd.DataFrame(probas)
    df['tweet'] = processeddf['OriginalTweet']
    df['Sentiment'] = processeddf['PredictedSentiment']
    # print(df.shape)
    # print(df['tsne_1'].head(2))
    fig = px.scatter(df, x='probas', y='tsne_1', hover_name= 'tweet',color='Sentiment',size_max=45
                     , template='plotly_white', title='Bigram similarity per class', labels={'words': 'Avg. Length<BR>(words)'}
                     , color_continuous_scale=px.colors.sequential.Sunsetdark)
    fig.update_traces(marker=dict(line=dict(width=1, color='Gray')))
    fig.update_xaxes(visible=False)
    fig.update_yaxes(visible=False)
    fig.show()
elif value == '1':
    t = input("Please Enter your Tweet about Covid-19:\n")
    t = processSingle(t)[0]
    print("1: SVM")
    print("2: Logistic Regression")
    print("3: Naive Bayes")
    m = input("Please Choose the number of model you want:\n")
    loaded_model = None
    if m == '1':
        loaded_model = joblib.load('savedModels/SVC.sav')

    elif m == '3':
        loaded_model = joblib.load('savedModels/nb.sav')

    elif m == '2':
        loaded_model = joblib.load('savedModels/LR.sav')

    pred = loaded_model.predict(t)

    if pred == 1:
        print("Positive")
    elif pred== -1:
        print("Negative")
    elif pred== 0:
        print("Neutral")






